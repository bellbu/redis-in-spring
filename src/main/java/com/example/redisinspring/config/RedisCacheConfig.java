...
public class RedisCacheConfig { // Redis 캐시 설정
    ...
    @Bean
    public CacheManager boardCacheManager(RedisConnectionFactory redisConnectionFactory) { // RedisConfig에서 빈으로 등록된 LettuceConnectionFactory 의존성 주입
        RedisCacheConfiguration redisCacheConfiguration = RedisCacheConfiguration
                // Redis 캐시에 저장할 데이터 기본 설정
                .defaultCacheConfig()
                // Redis 캐시에 Key를 저장할 때 String 형태로 직렬화(변환)해서 저장
                .serializeKeysWith(
                        RedisSerializationContext.SerializationPair.fromSerializer(
                                new StringRedisSerializer()
                        )
                )
                // Redis 캐시에 Value를 저장할 때 Json 형태로 직렬화(변환)해서 저장
                .serializeValuesWith(
                        RedisSerializationContext.SerializationPair.fromSerializer(
                ...
                // 캐시 데이터 유효기간(TTL) 설정
                .entryTtl(Duration.ofMinutes(1L));  // 유효기간 1분 설정
